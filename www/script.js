(() => {
    ...
    function calculateStats() {
        ...
        // Get all Hayd Start Dates
        // We look for entries where status is 'hayd'
        const haydStarts = App.history.filter(e => e.status === 'hayd').map(e => new Date(e.time));

        if (haydStarts.length < 2) return;

        // 1. Calculate Cycle Length (Start to Start)
        let totalCycleMs = 0;
        let cycleCount = 0;

        // History is sorted Newest -> Oldest.
        // Cycle = Time between Newest Hayd and Previous Hayd
        for (let i = 0; i < haydStarts.length - 1; i++) {
            const diff = haydStarts[i] - haydStarts[i + 1];
            totalCycleMs += diff;
            cycleCount++;
        }
        App.avgCycleLength = totalCycleMs / cycleCount;

        // 2. Calculate Hayd Duration
        // Rough estimate: look at history pairs
        let totalHaydMs = 0;
        let haydCount = 0;
    ...
    // 3. CALENDAR RENDERER (Updated with Predictions)
    ...
    function renderCalendar() {
        ...
        if (App.avgCycleLength > 0 && App.history.length > 0) {
            ...
            if (lastHayd) {
                const lastDate = new Date(lastHayd.time);
                // Predict the NEXT start
                const nextDateMs = lastDate.getTime() + App.avgCycleLength;
                predStart = new Date(nextDateMs);
                // Duration of prediction (use avg hayd length, default 5 days)
        ...
        for (let day = 1; day <= daysInMonth; day++) {
            const currentDayDate = new Date(year, month, day, 12, 0, 0); // Noon to avoid timezone edge cases
            ...
            if (predStart && currentDayDate >= predStart && currentDayDate <= predEnd) {
                // Only show prediction if we don't have actual data yet (future)
                if (currentDayDate > now) isPredicted = true;
            }

            let bgClass = "", textClass = "", borderClass = "";

            if (currentDayDate > now && isPredicted) {
                // PREDICTION STYLE (Gray Dashed)
    ...
    // ... (Keep the rest of the file: updateStatusUI, formatDateTime, renderHistory, etc.) ...
    // ... Copy everything from previous version below here ...

    // (I will paste the rest of the critical functions below to ensure nothing is lost)
    ...
    // ... (Keep saveFasting, updateDebt, updatePaid, updateFastingUI, openFasting, closeFasting EXACTLY as before) ...
    ...
    // NOTE: Removed old calculateAverages(), logic is now inside calculateStats() called by renderCalendar()
    // but openInsights calls calculateAverages() usually. Let's redirect it.
